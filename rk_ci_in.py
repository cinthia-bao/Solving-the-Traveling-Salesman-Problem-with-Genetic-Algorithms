...
tolerancia = .05
name = 'Generaciones_rk_ci_in.json'
import sys
import json
...
# Importar metodos
from metodos_seleccion import rank
from metodos_cruza import cruza_ciclo
from metodos_mutacion import inseption
...
def evolucion(poblacion, tolerancia,ruta):
    ...
    while tolerancia < delta_ap:
        # Seleccion
        padres1, padres2 = rank(poblacion)
        # Cruza
        hijo1, hijo2 = cruza_ciclo(padres1, padres2)
        poblacion = padres1+padres2+hijo1+hijo2
        poblacion, aptitud, distance = aptitud_calculate(poblacion)
        # Mutacion
        poblacion = inseption(poblacion[:int(numero)], 5*2)
        poblacion, aptitud, distance = aptitud_calculate(poblacion)
